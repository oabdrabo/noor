...
import pandas as pd
import numpy as np
import json, os, io, base64, sqlite3, re, logging
...
def search_quran_vec(db, query_embedding, limit):
    ...
    ph = ','.join('?' * len(rowids))
    meta = {m[0]: m for m in db.execute(
        f'SELECT verse_id, surah, ayah, text, translation FROM quran_metadata WHERE verse_id IN ({ph})', rowids
    ).fetchall()}
    return [{'surah': meta[rid][1], 'ayah': meta[rid][2], 'text': meta[rid][3],
             'translation': meta[rid][4], 'score': max(0.0, 1.0 - distances[rid])}
            for rid in rowids if rid in meta]
...
def search_hadith_vec(db, query_embedding, limit):
    ...
    ph = ','.join('?' * len(rowids))
    meta = {m[0]: m for m in db.execute(
        f'SELECT hadith_id, collection_name, hadith_number, text, reference FROM hadith_metadata WHERE hadith_id IN ({ph})', rowids
    ).fetchall()}
    return [{'collection': meta[rid][1], 'hadith_number': meta[rid][2], 'text': meta[rid][3],
             'reference': meta[rid][4], 'score': max(0.0, 1.0 - distances[rid])}
            for rid in rowids if rid in meta]
...
def _ensure_model_version(db, model_name):
    ...
    if stored is None or stored[0] != model_name:
        print(f"[{datetime.now()}] Model changed ({stored[0] if stored else 'none'} -> {model_name}), rebuilding...", flush=True)
        for t in ['quran_vec', 'hadith_vec']:
            db.execute(f'DROP TABLE IF EXISTS {t}')
        db.execute('DELETE FROM quran_metadata')
        db.execute('DELETE FROM hadith_metadata')
        db.commit()
    db.execute(f'CREATE VIRTUAL TABLE IF NOT EXISTS quran_vec USING vec0(embedding float[{EMBED_DIM}] distance_metric=cosine)')
    db.execute(f'CREATE VIRTUAL TABLE IF NOT EXISTS hadith_vec USING vec0(embedding float[{EMBED_DIM}] distance_metric=cosine)')
    ...
    def _run():
        try:
            ...
            if existing >= total:
                ...
                print(f"[{datetime.now()}] {action} quran index ({existing}/{total})", flush=True)
...
async def islamic_qa(query: QAQuery, request: Request):
    s = request.app.state
    if not s.quran_ready:
        return _indexing_resp(s, answer="System is indexing, please wait...", relevant_verses=[])
...
async def count_word_occurrences(query: CountQuery, request: Request):
    ...
    try:
        ...
        return {"word": word, "count": int(counts.sum()), "examples": examples, "total_verses_with_word": int(mask.sum())}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_theme_clusters(request: Request):
    ...
    if 'themes' not in s.cache:
        ...
        sc, ac = _col(sample, 'surah_no', 'surah'), _col(sample, 'ayah_no_surah', 'ayah')
        texts = (sample[ar].fillna('').astype(str) + ' ' + sample[en].fillna('').astype(str)).tolist()
        embeddings = np.array(s.model.encode(texts, batch_size=32), dtype=np.float32)
        labels = _kmeans()(n_clusters=15, random_state=42, n_init=10).fit_predict(embeddings)
...
async def get_surah_verses(surah_no: int, request: Request):
    ...
    return {
        "surah": surah_no, "surah_name": f"Surah {surah_no}",
        "total_verses": len(sv),
        "verses": [{"ayah": int(r[ac]), "text": r.get('ayah_ar', ''), "translation": r.get('ayah_en', '')}
